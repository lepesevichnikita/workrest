...
export class Jobs extends Page {
  ...
  _loadData() {
    jobService.getJobs().then(response => {
      ...
      $.get(templateHelper.getTemplatePath(this._cardTemplateName),
            cardBody => {
              $.tmpl(cardBody, jobs).appendTo(this._containerSelector);
              ...
              $.get(templateHelper.getTemplatePath(this._popupTemplateName),
                    popupBody => {
                      $(this._modalsSelector).remove();
                      $.tmpl(popupBody, jobs).appendTo(this._containerSelector);
                      $(this._containerSelector).dimmer('hide');
                      $(this._cardSelector).unbind();
                      $(this._cardSelector).click(function(event) {
            event.preventDefault();
                        const id = $(this).attr('id');
                        $(this._getModalSelectorById(id)).modal('show');
          });
        });
      });
    });
  ...
  process() {
    replacePage('jobs').then(this._loadData()).then(() => super.process());
  }
}
